...
{\centering \includegraphics[width=\maxwidth]{figures/knitr-kmeansexamplecode-1} 
...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# quanteda version 0.9.9.65}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Using 3 of 4 cores for parallel computing}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# \\\#\# Attaching package: 'quanteda'}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# The following object is masked from 'package:utils':\\\#\# \\\#\#\ \ \ \  View}}\begin{alltt}
...
\hlstd{trump.dfm.trim} \hlkwb{<-} \hlkwd{dfm_trim}\hlstd{(trump.dfm,} \hlkwc{min_count} \hlstd{=} \hlnum{3}\hlstd{,} \hlkwc{min_docfreq} \hlstd{=} \hlnum{2}\hlstd{)}

\hlcom{# some guesses for "optimal k is sqrt of number of documents}
\hlstd{k} \hlkwb{<-} \hlkwd{round}\hlstd{(}\hlkwd{sqrt}\hlstd{(}\hlkwd{ndoc}\hlstd{(trump.dfm.trim)}\hlopt{/}\hlnum{2}\hlstd{))}

\hlcom{#a bit too many}
\hlstd{k}
\end{alltt}
\begin{verbatim}
## [1] 34
\end{verbatim}
\begin{alltt}
\hlcom{#computing words as shares of words in total document is like normalising length of documents, so can use Euclidian distance.}

\hlcom{#tf function converts word count dfm to share}
\hlstd{clusterk5} \hlkwb{<-} \hlkwd{kmeans}\hlstd{(}\hlkwd{tf}\hlstd{(trump.dfm.trim,} \hlstr{"prop"}\hlstd{),} \hlnum{5}\hlstd{)}
\hlstd{splits}\hlkwb{<-}\hlkwd{split}\hlstd{(}\hlkwd{docnames}\hlstd{(trump.dfm.trim), clusterk5}\hlopt{$}\hlstd{cluster)}
\hlkwd{unlist}\hlstd{(}\hlkwd{lapply}\hlstd{(splits, length))}
\end{alltt}
\begin{verbatim}
##    1    2    3    4    5 
##   56  347  100 1690  135
...
\begin{alltt}
\hlcom{## k-means clustering}
\hlkwd{textplot_wordcloud}\hlstd{(}\hlkwd{dfm_trim}\hlstd{(trump.dfm.trim[splits[[}\hlnum{3}\hlstd{]]],} \hlkwc{min_count} \hlstd{=} \hlnum{1}\hlstd{))}
\end{alltt}
\end{kframe}
