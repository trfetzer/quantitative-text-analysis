...
\begin{alltt}
\hlstd{REFERENCE} \hlkwb{<-} \hlkwd{corpus}\hlstd{(}\hlkwd{readtext}\hlstd{(}\hlstr{"../../Data/Speeches/*.txt"}\hlstd{,} \hlkwc{docvarsfrom} \hlstd{=} \hlstr{"filenames"}\hlstd{))}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in corpus(readtext("{}../../Data/Speeches/*.txt"{}, docvarsfrom = "{}filenames"{})): could not find function "{}readtext"{}}}\begin{alltt}
\hlkwd{summary}\hlstd{(REFERENCE)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in summary(REFERENCE): object 'REFERENCE' not found}}\end{kframe}
\end{knitrout}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{REFERENCE.dfm} \hlkwb{<-} \hlkwd{dfm}\hlstd{(REFERENCE,} \hlkwc{toLower} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{removeNumbers} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{removePunct} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{removeSeparators} \hlstd{=} \hlnum{TRUE}\hlstd{,}
    \hlkwc{stem} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{ignoredFeatures} \hlstd{=} \hlkwd{stopwords}\hlstd{(}\hlstr{"english"}\hlstd{))}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in dfm(REFERENCE, toLower = TRUE, removeNumbers = TRUE, removePunct = TRUE, : object 'REFERENCE' not found}}\begin{alltt}
\hlcom{## first 7 elements are clinton speeches, next 6 are Trump}
\hlstd{refscores} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlkwd{rep}\hlstd{(}\hlopt{-}\hlnum{1}\hlstd{,} \hlnum{7}\hlstd{),} \hlkwd{rep}\hlstd{(}\hlnum{1}\hlstd{,} \hlnum{6}\hlstd{))}
\hlstd{ws} \hlkwb{<-} \hlkwd{textmodel}\hlstd{(REFERENCE.dfm, refscores,} \hlkwc{model} \hlstd{=} \hlstr{"wordscores"}\hlstd{,} \hlkwc{smooth} \hlstd{=} \hlnum{1}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in textmodel(REFERENCE.dfm, refscores, model = "{}wordscores"{}, smooth = 1): object 'REFERENCE.dfm' not found}}\begin{alltt}
\hlcom{## most trumpy words}
\hlstd{ws}\hlopt{@}\hlkwc{Sw}\hlstd{[ws}\hlopt{@}\hlkwc{Sw} \hlopt{>} \hlnum{0.5}\hlstd{][}\hlnum{1}\hlopt{:}\hlnum{15}\hlstd{]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'ws' not found}}\begin{alltt}
\hlcom{## most hillary words}
\hlstd{ws}\hlopt{@}\hlkwc{Sw}\hlstd{[ws}\hlopt{@}\hlkwc{Sw} \hlopt{< -}\hlnum{0.5}\hlstd{][}\hlnum{1}\hlopt{:}\hlnum{15}\hlstd{]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'ws' not found}}\end{kframe}
\end{knitrout}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{## scoring the reference documents}
\hlkwd{predict}\hlstd{(ws)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in predict(ws): object 'ws' not found}}\end{kframe}
\end{knitrout}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{## rescale the scores}
\hlkwd{predict}\hlstd{(ws,} \hlkwc{rescaling} \hlstd{=} \hlstr{"lbg"}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in predict(ws, rescaling = "{}lbg"{}): object 'ws' not found}}\end{kframe}
\end{knitrout}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{ws2} \hlkwb{<-} \hlkwd{textmodel}\hlstd{(REFERENCE.dfm, refscores,} \hlkwc{model} \hlstd{=} \hlstr{"wordscores"}\hlstd{,} \hlkwc{smooth} \hlstd{=} \hlnum{0}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in textmodel(REFERENCE.dfm, refscores, model = "{}wordscores"{}, smooth = 0): object 'REFERENCE.dfm' not found}}\begin{alltt}
\hlcom{## most trumpy words}
\hlstd{ws2}\hlopt{@}\hlkwc{Sw}\hlstd{[ws2}\hlopt{@}\hlkwc{Sw} \hlopt{==} \hlnum{1}\hlstd{][}\hlnum{1}\hlopt{:}\hlnum{15}\hlstd{]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'ws2' not found}}\begin{alltt}
\hlcom{## most hillary words}
\hlstd{ws2}\hlopt{@}\hlkwc{Sw}\hlstd{[ws2}\hlopt{@}\hlkwc{Sw} \hlopt{== -}\hlnum{1}\hlstd{][}\hlnum{1}\hlopt{:}\hlnum{15}\hlstd{]}
\end{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'ws2' not found}}\begin{alltt}
\hlkwd{predict}\hlstd{(ws2)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in predict(ws2): object 'ws2' not found}}\end{kframe}
\end{knitrout}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# LOAD SOME OTHER SPEECHES, TURN INTO DFM AND PREDICT BASED ON WORD SCORES}
\hlstd{TEXT} \hlkwb{<-} \hlkwd{readLines}\hlstd{(}\hlkwc{con} \hlstd{=} \hlstr{"../../Data/speeches-2016-election.json"}\hlstd{)}
\hlstd{VIRGIN} \hlkwb{<-} \hlkwd{lapply}\hlstd{(TEXT,} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)} \hlkwd{data.frame}\hlstd{(}\hlkwd{fromJSON}\hlstd{(x)))}
\hlstd{VIRGIN} \hlkwb{<-} \hlkwd{data.table}\hlstd{(}\hlkwd{rbindlist}\hlstd{(VIRGIN))}
\hlstd{VIRGIN} \hlkwb{<-} \hlstd{VIRGIN[}\hlkwd{year}\hlstd{(date)} \hlopt{>} \hlnum{2008}\hlstd{]}
\hlstd{VIRGIN} \hlkwb{<-} \hlstd{VIRGIN[,} \hlkwd{list}\hlstd{(}\hlkwc{text} \hlstd{=} \hlkwd{paste}\hlstd{(text,} \hlkwc{collapse} \hlstd{=} \hlstr{" "}\hlstd{)),} \hlkwc{by} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"speaker_name"}\hlstd{,} \hlstr{"speaker_party"}\hlstd{)]}
\hlstd{VIRGIN.corp} \hlkwb{<-} \hlkwd{corpus}\hlstd{(VIRGIN}\hlopt{$}\hlstd{text)}
\hlkwd{docnames}\hlstd{(VIRGIN.corp)} \hlkwb{<-} \hlstd{VIRGIN}\hlopt{$}\hlstd{speaker_name}

\hlstd{VIRGIN.dfm} \hlkwb{<-} \hlkwd{dfm}\hlstd{(VIRGIN.corp,} \hlkwc{toLower} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{removeNumbers} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{removePunct} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{removeSeparators} \hlstd{=} \hlnum{TRUE}\hlstd{,}
    \hlkwc{stem} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{ignoredFeatures} \hlstd{=} \hlkwd{stopwords}\hlstd{(}\hlstr{"english"}\hlstd{))}

\hlkwd{predict}\hlstd{(ws2, VIRGIN.dfm)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in predict(ws2, VIRGIN.dfm): object 'ws2' not found}}\begin{alltt}
\hlstd{VIRGIN}\hlopt{$}\hlstd{speaker_party}
\end{alltt}
\begin{verbatim}
## [1] "D" "R" "I" "D" "D" "R" "R" "R" "R"
\end{verbatim}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in dfm(REFERENCE, toLower = TRUE, removeNumbers = TRUE, removePunct = TRUE, : object 'REFERENCE' not found}}\begin{alltt}
\hlstd{bs} \hlkwb{<-} \hlkwd{textmodel}\hlstd{(REFERENCE.dfm, refscores,} \hlkwc{model} \hlstd{=} \hlstr{"NB"}\hlstd{,} \hlkwc{smooth} \hlstd{=} \hlnum{1}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in textmodel(REFERENCE.dfm, refscores, model = "{}NB"{}, smooth = 1): object 'REFERENCE.dfm' not found}}\begin{alltt}
\hlcom{## F_ij are presented in this matrix - the probability of a word given class}
\hlkwd{head}\hlstd{(}\hlkwd{t}\hlstd{(bs}\hlopt{$}\hlstd{PwGc))}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in t(bs\$PwGc): object 'bs' not found}}\begin{alltt}
\hlcom{# Hillary}
\hlkwd{sort}\hlstd{(}\hlkwd{log}\hlstd{(bs}\hlopt{$}\hlstd{PwGc[}\hlnum{1}\hlstd{, ]}\hlopt{/}\hlstd{bs}\hlopt{$}\hlstd{PwGc[}\hlnum{2}\hlstd{, ]),} \hlkwc{decreasing} \hlstd{=} \hlnum{TRUE}\hlstd{)[}\hlnum{1}\hlopt{:}\hlnum{20}\hlstd{]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in sort(log(bs\$PwGc[1, ]/bs\$PwGc[2, ]), decreasing = TRUE): object 'bs' not found}}\begin{alltt}
\hlcom{# Trump}
\hlkwd{sort}\hlstd{(}\hlkwd{log}\hlstd{(bs}\hlopt{$}\hlstd{PwGc[}\hlnum{1}\hlstd{, ]}\hlopt{/}\hlstd{bs}\hlopt{$}\hlstd{PwGc[}\hlnum{2}\hlstd{, ]),} \hlkwc{decreasing} \hlstd{=} \hlnum{FALSE}\hlstd{)[}\hlnum{1}\hlopt{:}\hlnum{20}\hlstd{]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in sort(log(bs\$PwGc[1, ]/bs\$PwGc[2, ]), decreasing = FALSE): object 'bs' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in sort(log(bs\$PwGc[1, ]/bs\$PwGc[2, ]), decreasing = TRUE): object 'bs' not found}}\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# Hillary this ranking is identical when looking at the ratio of the posteriors}
\hlkwd{sort}\hlstd{(}\hlkwd{log}\hlstd{(bs}\hlopt{$}\hlstd{PcGw[}\hlnum{1}\hlstd{, ]}\hlopt{/}\hlstd{bs}\hlopt{$}\hlstd{PcGw[}\hlnum{2}\hlstd{, ]),} \hlkwc{decreasing} \hlstd{=} \hlnum{TRUE}\hlstd{)[}\hlnum{1}\hlopt{:}\hlnum{20}\hlstd{]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in sort(log(bs\$PcGw[1, ]/bs\$PcGw[2, ]), decreasing = TRUE): object 'bs' not found}}\end{kframe}
\end{knitrout}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
